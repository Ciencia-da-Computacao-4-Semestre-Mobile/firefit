...
class HiitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnIniciar.setOnClickListener {
            ...
            val tipo = when (selectedId) {
                R.id.rbCorrida -> "HIIT Full Body"
                R.id.rbBicicleta -> "HIIT Inferior"
                R.id.rbCaminhada -> "HIIT Superior"
                else -> "corrida"
            }
    ...
    private fun carregarLista(layoutRes: Int) {
        val listaExercicios = when (layoutRes) {
            R.layout.item_lista_corrida -> listOf("Burpee", "Mountain climbers", "Polichinelo")
            R.layout.item_lista_bicicleta -> listOf("Agachamento com salto", "Afundo alternado rápido", "Skater (patinador lateral)")
            R.layout.item_lista_caminhada -> listOf("Flexão", "Socos no ar (shadow boxing)", "Prancha com toque no ombro")
            else -> emptyList()
        }
